...
from backend.gcp_utils.gcs_bucket import download_file_from_gcs, upload_file_to_gcs, remove_file_from_gcs, clean_gcs_bucket
...
def run_inference_job() -> Dict:
    ...
    warnings.warn("run_inference_job is deprecated: use the orchestration pipeline jobs", DeprecationWarning)
...
def run_decrowding_job():
    ...
    if should_decrowd:
        ...
        output_dict["crowd"] = "crowd.mp3"
        output_dict["instrumental_(decrowd)"] = "instrumental_(decrowd).mp3"
        output_dict["gcs_links"]["instrumental_(decrowd)"] = f"https://storage.googleapis.com/{GCS_BUCKET}/{decrowd_blob}" 
        
        # write final_audio_path to GCS as JSON blob
        final_audio_path = decrowd_instrumental_path
        final_audio_gcs_link = write_json_to_gcs_blob(job_id=job_id, final_audio_path=final_audio_path)
