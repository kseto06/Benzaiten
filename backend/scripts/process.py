...
def run_karaoke_inference(
    model_name: str,
    audio_path: str,
    output_path: str = "./backend/tests/audio_outputs/",
) -> typing.IO[str]:
    """
    ...
    models = {
        "bs-roformer": (
            "./backend/models/BS-RoFormer/bs_roformer_karaoke.ckpt",
            "./backend/models/BS-RoFormer/bs_roformer_config_karaoke.yaml",
        ),
        "decrowd": (
            "./backend/models/MelBand-RoFormer-DeCrowd/mel_band_roformer_decrowd.ckpt",
            "./backend/models/MelBand-RoFormer-DeCrowd/model_mel_band_roformer_decrowd.yaml",
        ),
    }

    if model_name not in models:
        raise ValueError(
            f"Model '{model_name}' is not supported. Supported models are: {list(models.keys())}"
        )
    ...
    if model_name == "bs-roformer":
        ...
        ckpt = torch.load(
            hf_hub_download(
                repo_id="kseto06/benzaiten", filename="bs_roformer_karaoke.ckpt"
            ),
            map_location="cpu",
        )
        ...
        with open(
            hf_hub_download(
                repo_id="kseto06/benzaiten", filename="bs_roformer_config_karaoke.yaml"
            ),
            "r",
        ) as f:
        ...
        model = BSRoformer(
            ...
            multi_stft_resolution_loss_weight=model_cfg[
        ...
        if missing or unexpected:
            raise ValueError(
                f"State dict keys mismatch. Missing keys: {missing}, Unexpected keys: {unexpected}"
            )
        ...
        soundfile.write(
            os.path.join(output_path, "instrumental.mp3"), instrumental.T, sample_rate
        )
        ...
        ckpt = torch.load(
            hf_hub_download(
                repo_id="kseto06/benzaiten", filename="mel_band_roformer_decrowd.ckpt"
            ),
            map_location="cpu",
        )
        ...
        with open(
            hf_hub_download(
                repo_id="kseto06/benzaiten", filename="mel_band_roformer_decrowd.yaml"
            ),
            "r",
        ) as f:
            cfg = yaml.load(f, Loader=yaml.FullLoader)
        ...
        soundfile.write(
            os.path.join(output_path, "instrumental_(decrowd).mp3"),
            decrowd.T,
            sample_rate,
        )
...
if __name__ == "__main__":
    ...
    run_karaoke_inference(
        ...
        audio_path="./backend/tests/audio_files/i_miss_you.mp3",
    )
    run_karaoke_inference(
        model_name="decrowd", audio_path="./backend/tests/audio_files/instrumental.mp3"
    )
