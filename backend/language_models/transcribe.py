...
def transcribe(
    ...
    segments, info = model.transcribe(
        audio_path, 
        language=language if language != "mul" else None, #if language is multilingual, let the model detect the language
        task="transcribe",
        beam_size=5, 
    ...
    for segment in segments:
        ...
        results.append({
            ...
            "text": { #text, romanized, translated
                "original": text,
                "romanization": romanize(text=text, translate_language_code=language),
...
def run_srt_inference(audio_path: str, language: str, output_path: str = "./backend/tests/transcription_outputs") -> str:
    '''
    ...
    with open(output_path, 'w', encoding='utf-8') as f:
        for i, segment in enumerate(segments, start=1):
            ...
            f.write(f"{segment['text']['original']}\n"
...
if __name__ == "__main__":
    ...
    run_srt_inference(audio_path="./backend/tests/audio_outputs/vocals.mp3", language="mul")