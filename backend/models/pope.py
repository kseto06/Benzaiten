...
def apply_pope_to_qk(
    pope: PolarEmbedReturn, q, k, to_magnitude=F.softplus, return_complex=False
):
    freqs, bias = pope

    q_len, k_len, qk_dim, rotate_dim = (
        q.shape[-2],
        k.shape[-2],
        q.shape[-1],
        freqs.shape[-1],
...
class PoPE(Module):
    ...
    @autocast("cuda", enabled=False)
    def forward(self, pos_or_seq_len: Tensor | int, offset=0):
        ...
        bias = self.bias.clamp(-2.0 * pi, 0.0)
...
def compute_attn_similarity_non_fused(q, k, pope, head_dimension_at_first=True):
...
def compute_attn_similarity(q, k, pope, allow_tf32=True, head_dimension_at_first=True):
    assert divisible_by(
        q.shape[1 if head_dimension_at_first else 2],
        k.shape[1 if head_dimension_at_first else 2],
    )
    ...
    assert head_dim in {
        ...
        128,
        256,
    }, f"head_dim {head_dim} not in common sizes"
    ...
    if TRITON_AVAILABLE and is_cuda:
        ...
        return triton_compute_qk_similarity(
            q, k, freqs, bias, rotate_dim, allow_tf32=allow_tf32
        )

    return compute_attn_similarity_non_fused(
        q, k, pope, head_dimension_at_first=head_dimension_at_first
    )
...
def flash_attn_with_pope(
    ...
    out = F.scaled_dot_product_attention(
        q, k, v, attn_mask=attn_mask, is_causal=causal, scale=softmax_scale
    )
...
def triton_compute_qk_similarity():
    raise NotImplementedError(
        "Triton compute_qk_similarity is not implemented in this environment"
    )


def flash_attn():
    raise NotImplementedError(
        "Triton flash_attn is not implemented in this environment"
    )
